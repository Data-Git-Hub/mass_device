...
# --- Matplotlib backend selection (Windows: TkAgg is usually safest) ---
import matplotlib
try:
    if os.name == "nt":
...
# --- Source selection (Kafka or file tail) ---
...
class ConsumerApp:
    def __init__(self):
        ...
        self.times = collections.deque(maxlen=WINDOW)     # datetimes (LOCAL tz for plotting)
        self.temps_f = collections.deque(maxlen=WINDOW)   # °F
        self.press = collections.deque(maxlen=WINDOW)     # hPa
        ...
        # ===== Figure & axes (with extra right margin so label isn't clipped) =====
        ...
        self.fig.tight_layout(rect=(0.06, 0.12, 0.86, 0.90))  # right margin reduced from 0.98 -> 0.86
        ...
        self.ax1.set_ylabel("Temp (°F)")

        # Right y-axis label + ticks fully visible
        self.ax2.yaxis.set_label_position("right")
        self.ax2.yaxis.tick_right()
        self.ax2.set_ylabel("Pressure (hPa)", labelpad=18, rotation=270, va="bottom")
        ...
        self.ax1.legend(h1 + h2, l1 + l2, loc="lower right",
    ...
    def _source_iter(self):
        return _kafka_stream(self.cons) if self.cons else _tail_jsonl(INFILE)

    def _ingest_loop(self):
        try:
            for obj in self._source_iter():
                append_jsonl(CONSUMER_DB, obj)  # persist every consumed message
                self.q.put(obj, timeout=1.0)
        except Exception as e:
            log.error("Ingest thread error: %s", e)
        finally:
            self.running = False

    def _process_one(self, obj: Dict[str, Any]) -> None:
        # 1) Time: parse UTC ts_iso and convert to local for plotting
        try:
            ts_utc = datetime.strptime(obj["ts_iso"], "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        except Exception:
            ts_utc = datetime.fromisoformat(obj["ts_iso"].replace("Z", "+00:00"))
            if ts_utc.tzinfo is None:
                ts_utc = ts_utc.replace(tzinfo=timezone.utc)
        ts_local = ts_utc.astimezone()  # system local timezone
        self.times.append(ts_local)
        ...
        # (A) temperature anomaly on latest point
        if z_now.size:
            last_z = float(z_now[-1])
            if abs(last_z) >= ZTH:
                ctx["z_score"] = last_z
                _emit_alert(f"Temp anomaly |z|≥{ZTH} (z={last_z:.2f})", ctx)

        # (B) rapid pressure drop
        if self.last_pressure is not None and p_hpa == p_hpa:  # not NaN
        ...
        # 5) FIRST RECORD log immediately, then every 10th
    ...
    def _redraw(self) -> None:
        ...
        xs = date2num(list(self.times))
        t_arr = np.array(self.temps_f, dtype=float)
        p_arr = np.array(self.press, dtype=float)

        self.line_t.set_data(xs, t_arr)
        self.line_p.set_data(xs, p_arr)

        # X scale: pad nicely if only one point
        if len(xs) == 1:
            pad_days = 60.0 / 86400.0  # 60 seconds as days
            self.ax1.set_xlim(xs[0] - pad_days, xs[0] + pad_days)
        else:
            self.ax1.set_xlim(xs.min(), xs.max())

        # Y scales
        if t_arr.size:
            self.ax1.set_ylim(t_arr.min() - 2.0, t_arr.max() + 2.0)
        if p_arr.size:
            self.ax2.set_ylim(p_arr.min() - 1.5, p_arr.max() + 1.5)

        # Highlight anomalies (z on °C)
    ...
    def _on_timer(self, _):
        # Drain up to N msgs quickly; keeps UI snappy
        processed = 0
        while processed < 200:
            try:
